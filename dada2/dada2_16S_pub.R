...
preFilt_dir <- file.path(work_dir,"preFilt_16S")
primerCut5_dir <- file.path(work_dir,"primerCut5_16S")
primerCut3_dir <- file.path(work_dir,"primerCut3_16S")
qualFiltTrim_dir <- file.path(work_dir,"qualFiltTrim_16S")
...
REV_PRIMER="GGACTACHVGGGTWTCTAAT" #806R_1
...
dim(seqtab.nochim)
write.csv(t(seqtab.nochim), "~/path_out/seqtab_all_16S.csv", quote=FALSE ) # CHANGE ME to output directory.
saveRDS(filterOut, file = "~/path_out/seqtab_nochim_16S.RDA") # CHANGE ME to output directory.
...
taxa <- assignTaxonomy(seqtab.nochim, "~/path_to_db/silva_nr_v132_train_set.fa.gz", multithread=TRUE) # CHANGE ME to database directory.
taxa.print <- taxa # Removing sequence rownames for display only
rownames(taxa.print) <- NULL
head(taxa.print)
write.csv(cbind(t(seqtab.nochim), taxa.print), "~/path_out/assigned_all_16S.csv", quote=FALSE ) # CHANGE ME to output directory.
